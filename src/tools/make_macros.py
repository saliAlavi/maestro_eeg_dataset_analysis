...
# name are the same model and are read together.
...
def emit_external():
    ...
    """
    out = []
    f = os.path.join(EXT, "external_probe_sweep.csv")
    if os.path.exists(f):
    ...
    f = os.path.join(EXT, "external_priors.csv")
    if os.path.exists(f):
        for _, r in pd.read_csv(f).iterrows():
...
def main():
    ...
    df = load(a.pattern)
    probes = sep = None
    f = os.path.join(PROJ, "credit_candidate_probes.csv")
    if os.path.exists(f):
        probes = pd.read_csv(f)
    f = os.path.join(PROJ, "credit_role_separation.csv")
    if os.path.exists(f):
        sep = pd.read_csv(f)
