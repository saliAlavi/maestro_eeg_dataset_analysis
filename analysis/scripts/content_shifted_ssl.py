...
SHIFTED match-mismatch (loudness-controlled): same attended talker, time-shifted.
...
def load():
    ...
    for s in range(1, 17):
        ...
        att = z["env"][np.arange(len(y)), y, :].mean(1).astype(np.float32)  # (N,T) attended talker broadband
        if shuffle:                                   # EEG-shuffle control: break EEG<->cand pairing
            eeg = eeg[rng0.permutation(len(y))]
        D[s] = (eeg, zs(att, 1), y)
    T = min(D[s][0].shape[-1] for s in D)
    return {s: (e[:, :, :T], b[:, :T], y) for s, (e, b, y) in D.items()}   # b is (N,T) attended env


def permute(att, y, seed, n_cand=4, min_sep=192):
    """Loudness-controlled: matched attended envelope + (n_cand-1) time-shifted copies of
    the SAME envelope, in random order. min_sep=192 -> >=3 s shift @64 Hz."""
    rng = np.random.default_rng(seed)
    N, T = att.shape; cand = np.empty((N, n_cand, T), np.float32); tgt = np.empty(N, int)
    for i in range(N):
        cs = [att[i]] + [np.roll(att[i], int(rng.integers(min_sep, T - min_sep))) for _ in range(n_cand - 1)]
        cs = np.stack(cs); p = rng.permutation(n_cand); cand[i] = cs[p]; tgt[i] = int(np.where(p == 0)[0][0])
    return cand, tgt
