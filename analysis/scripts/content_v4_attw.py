...
ATTW = os.environ.get("ATTW", "1") == "1"
W_WRONG = float(os.environ.get("W_WRONG", "0.3"))
BASE = "/fs/ess/PAS2301/Data/AAD Data Collection/experiment_data"
...
def load():
    ...
    for s in range(1, 17):
        ...
        comp = {}
        try:
            a = json.load(open(f"{BASE}/Subject {s}/answers.json"))
            comp = {int(e["Trial No."]): int(e["Correct"]) for e in a if "Trial No." in e and "rain" not in e["Trial No."]}
        except Exception: pass
        tk = z["trial_k"].astype(int)
        w = np.array([1.0 if comp.get(int(t), 1) == 1 else W_WRONG for t in tk], np.float32) if ATTW else np.ones(len(y), np.float32)
        D[s] = (eeg, cand, y, w)
    T = min(D[s][0].shape[-1] for s in D)
    return {s: (e[:, :, :T], {k: v[:, :, :, :T] for k, v in c.items()}, y, w) for s, (e, c, y, w) in D.items()}
...
def train(model, Xe, cand, tgt, w, epochs, lr=1e-3, wd=1e-3, bs=48):
    model.to(dev).train(); opt = torch.optim.AdamW(model.parameters(), lr, weight_decay=wd)
    Xe = torch.as_tensor(Xe, device=dev); cand = to_dev(cand); tgt = torch.as_tensor(tgt, device=dev); w = torch.as_tensor(w, device=dev)
    for _ in range(epochs):
        for b in _bt(len(tgt), bs):
            ...
            ce = (F.cross_entropy(logits, tgt[bi], reduction="none") * w[bi]).sum() / (w[bi].sum() + 1e-6)
            loss = ce + rl
...
def main():
    ...
    for sd in seeds:
        ...
        for s in subs:
            ...
            oC = {k: np.concatenate([P[o][0][k] for o in subs if o != s]) for k in SPACES}; ot = np.concatenate([P[o][1] for o in subs if o != s]); ow = np.concatenate([Wt[o] for o in subs if o != s])
            pre = train(MultiNet(dims), oE, oC, ot, ow, 20)
            cpre = MultiNet(dims); cpre.enc.load_state_dict(ssl_state); cpre = train(cpre, oE, oC, ot, ow, 20)
            ...
            for tr, te in StratifiedKFold(5, shuffle=True, random_state=0).split(Xe, tgt):
                ...
                a["scratch"].append(evalm(train(MultiNet(dims), Xe[tr], trC, tgt[tr], Wt[s][tr], 50), Xe[te], teC, tgt[te])[0])
                mt = MultiNet(dims); mt.load_state_dict(pre.state_dict())
                ac, mw = evalm(train(mt, Xe[tr], trC, tgt[tr], Wt[s][tr], 20, lr=5e-4), Xe[te], teC, tgt[te]); a["transfer"].append(ac); lastmw = mw
                mc = MultiNet(dims); mc.load_state_dict(cpre.state_dict())
                a["combo"].append(evalm(train(mc, Xe[tr], trC, tgt[tr], Wt[s][tr], 20, lr=5e-4), Xe[te], teC, tgt[te])[0])
    ...
    print(f"\n=== content_v4_attw (attention-weighted) ({SPACES}) MEAN (chance .25; spectrogram-only=0.39) ===")
    for m in res:
