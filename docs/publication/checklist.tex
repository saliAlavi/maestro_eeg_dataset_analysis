...
\item {\bf Experiment statistical significance}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: All pooled accuracies are accompanied by 95\,\% bootstrap CIs (resampling the 16 per-subject accuracies with replacement). Sec.~\ref{sec:iter8} reports paired Wilcoxon signed-rank $p$-values against the raw condition.

\item {\bf Experiments compute resources}
    \item[] Question: For each experiment, does the paper provide sufficient information on the computer resources (type of compute workers, memory, time of execution) needed to reproduce the experiments?
    \item[] Answer: \answerYes{}
    \item[] Justification: All submitted baselines run CPU-only on an OSC SLURM \texttt{batch} partition, one task per (subject$\times$backbone), 8 CPUs and 32\,GB memory per task, 2\,h wall-time limit. A full benchmark sweep of the 16 subjects $\times$ 4 TRF backbones $+$ spectral $+$ gaze $+$ IMU $+$ video fits in $\sim$20 minutes wall-clock when parallelised across nodes. Deep-model baselines listed under Sec.~\ref{sec:todo} are GPU-bound and not included in the submitted numbers.
