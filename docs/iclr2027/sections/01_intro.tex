...
read that response from EEG, find the attended talker, and steer a hearing device
toward them \citep{pan2024neuroheed}.
...
produced it; a decoder that reads only behaviour can look, from its accuracy,
exactly like one that reads the brain \citep{rotaru2024}.
...
\citep{accou2021,monesi2020,puffay2023}. We show that this task can be solved
without using the brain signal at all, in three ways (Section~\ref{sec:failures}).
...
\textbf{(ii) The standard scoring function has a shortcut.} If the brain encoder
outputs the same vector at every time step, the usual score turns into a
classifier on the audio alone, and training often finds this solution.
...
We work on MAESTRO \citep{maestro2026}, a public multimodal AAD dataset released
with a benchmark. The benchmark already reports a permutation test: it shuffles
the EEG, gaze, head motion and video \emph{together}, and its contributions are
positive. That shows the listener's signals are used. It cannot show \emph{which}
...
compute each stream's Shapley value over all $2^M$ subsets of intact streams.
...
lands exactly at chance. Heads that read gaze and head motion get no audio, so
they cannot use audio cues. The candidate audio is tested before training.
