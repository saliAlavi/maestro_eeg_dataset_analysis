...
vector $\hat c$ at every step. Then the time average splits apart and the score
becomes $w^\top(\hat c \odot \frac1T\sum_t \hat a_{k,t}) + b$: a linear classifier
on the averaged audio, with exactly the power needed to read the audio mark of
(i). So the architecture contains an audio-only decoder, which it reaches by
letting the brain encoder go constant --- and this solution is easier to find
than the intended one. Problem (i) makes the audio shortcut \emph{enough}; problem
(ii) makes it \emph{reachable}.
